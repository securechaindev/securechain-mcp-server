...
async def get_tixs_tool(
    ...
    try:
        ...
        tixs = await tix_service.read_tixs_by_owner_name(owner, name, sbom_name)
        return [TextContent(type="text", text=dumps(json_encoder.encode({"tixs": tixs}), ensure_ascii=False, indent=2))]
